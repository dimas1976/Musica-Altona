<?php
/**
Template Name: Dozenten
 */

get_header(); ?>

<div id="lecturers" class="mt-4">
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/dumisani.jpg" class="img-fluid dumisani lazyload" alt="Dumisani Mabaso">
                    <div class="credit">Foto: &copy; EleonoraCucina</div>
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3 dumisani-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>

                        Musica Altona ist die Tür und das
                        Fenster zur Musik.

                        <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Dumisani Mabaso</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3 ingrid-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        <i class="fab fa-angellist"></i>
                        <sub class="pl-2">
                            <i class="fas fa-quote-right"></i>
                        </sub>
                    </blockquote>
                    <p>Ingrid Schöller</p>
                </div>
                <div class="col-md-4 order-first order-md-last text-md-center">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/ingrid.jpg" class="img-fluid ingrid lazyload" alt="Ingrid Schöller">
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/olle.jpg" class="img-fluid olle lazyload" alt="Olle Wolski">
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3 olle-qoutes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup> Musica Altona ist sozial, gutes
                        Team, in meinem Stadtteil Altona,
                        tolle Musikfeste, Persönlichkeits-
                        bildung und Aufbau sowie sichtbare
                        Freude und Motivation der
                        SchülerInnen. <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Olle Wolski</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3 willie-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>Für mich bedeutet Musica Altona die Möglichkeit, innerhalb einer bunten Vielfalt von Kolleginnen und Kollegen mit Kindern zu musizieren und so dazu beizutragen, dass die Freude an der Musik die Gegenwart und Zukunft der Kinder bereichern kann.<sub class="pl-2"><i
                            class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Willie Jakob</p>
                </div>
                <div class="col-md-4 order-first order-md-last text-md-center">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/willi.jpg" class="img-fluid willi lazyload" alt="Willie Jakob">
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/oliver.jpg" class="img-fluid oliver lazyload" alt="Oli Höppner">
                    <div class="credit">Foto: &copy; EleonoraCucina</div>
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3 oliver-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup> die Kinder kommen freiwillig
                        zum Unterricht.<br>
                        Das spricht für sich. <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Oli Höppner</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3 pablo-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>Musica Altona fühlt sich an wie
                        Elektrizität <sub class="pl-2"><i
                            class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Pablo Ruiz</p>
                </div>
                <div class="col-md-4 order-first order-md-last text-md-center">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/pablo.jpg" class="img-fluid pablo lazyload" alt="Pablo Ruiz">
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/heidi.jpg" class="img-fluid heidi lazyload" alt="Heidi Martens">
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3 oliver-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Musica Altona als Brücke, eine besondere Form der Kommunikation – unabhängig vom Alter oder kulturellem Hintergrund, Musica Altona verbindet <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Heidi Martens</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3 krischa-quotes">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        die Kinder kommen aus vielen Richtungen und Schulen. Beim Unterricht brauchen sie kein Handy. Die Zeit vergeht so schnell und am Ende der Stunde gibt es einen guten, gemeinsamen Sound. <sub class="pl-2"><i
                            class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Krischa Weber</p>
                </div>
                <div class="col-md-4 order-first order-md-last text-md-center">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/krischa.jpg" class="img-fluid krischa lazyload" alt="Krischa Weber">
                    <div class="credit">Foto: &copy; Eibe Krebs</div>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/gino.jpg" class="img-fluid gino lazyload" alt="Gino Ramirez">
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Wir bleiben dran! Es lohnt sich! Wir sind schon lange zusammen als 'Musikfreunde' <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Gino Ramirez</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        In Musica Altona vereinen sich ein sozialer und ein künstlerischer Anspruch. Im Zentrum steht die Freude am gemeinsamen Musizieren. Das finde ich wunderbar und bin sehr froh, dass ich dabei bin. <sub class="pl-2"><i
                            class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Jan Feddersen</p>
                </div>
                <div class="col-md-4 order-first order-md-last text-md-center">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/jan.jpg" class="img-fluid jan lazyload" alt="Jan Feddersen">
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/serena.jpg" class="img-fluid serena lazyload" alt="Serena Kahnert">
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Musica Altona bietet Kindern und Jugendlichen eine tolle Chance, ihre Lebensqualität durch das Instrumentalspiel deutlich zu verbessern. Großes Engagement auf allen Seiten.<sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Serena Kahnert</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Musica Altona ist wie eine Musikalische Reise um die Welt. Wie ein Regenbogen, der uns immer wieder verzaubert!
                        <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Hüseyin Duman</p>
                </div>
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/huseyin.jpg" class="img-fluid huseyin lazyload" alt="Hueseyin Duman">
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/thando.jpg" class="img-fluid thando lazyload" alt="Thando Walbaum">
                    <div class="credit">Foto: &copy; Thomas Leidig</div>
                </div>
                <div class="col-md-8 pt-2 pt-md-5 pb-5 pb-md-3 text-left">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Musica Altona bietet musikalische aber auch soziale Möglichkeiten, mit denen Kinder aufwachsen sollten.
                        <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Thando Walbaum</p>
                </div>
            </div>
        </div>
    </div>
    <div class="mb-md-2" data-aos="zoom-in">
        <div class="container">
            <div class="row">
                <div class="col-md-8 order-last order-md-first pt-2 pt-md-5 pb-5 pb-md-3">
                    <blockquote>
                        <sup class="pr-2"><i class="fas fa-quote-left"></i></sup>
                        Musik fördert in einzigartiger Weise das harmonische Zusammenwirken von Körper, Seele und Geist; wie auch das Zusammenleben mit anderen Menschen. Hier langfristig zu wachsen schenkt Musica Altona allen Kindern im Stadtteil.
                        <sub class="pl-2"><i class="fas fa-quote-right"></i></sub>
                    </blockquote>
                    <p>Mike Rutledge</p>
                </div>
                <div class="col-md-4">
                    <img data-src="<?= get_bloginfo("template_url"); ?>/img/dozenten/mike.jpg" class="img-fluid mike lazyload" alt="Mike Rutledge">
                </div>
            </div>
        </div>
    </div>
</div>

<?php get_footer();?>
